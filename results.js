...
console.log('Γòæ   RESULTS.JS LOADED - DIAGNOSTIC MODE Γòæ');
...
console.log('≡ƒÉë Checking Chinese zodiac data availability...');
if (typeof CHINESE_ZODIAC_DATA !== 'undefined') {
    console.log(`Γ£à CHINESE_ZODIAC_DATA loaded: ${CHINESE_ZODIAC_DATA.length} entries`);
} else {
    console.error('Γ¥î CHINESE_ZODIAC_DATA not found! Using compact fallback');
}

if (typeof findChineseZodiac !== 'undefined') {
    console.log('Γ£à findChineseZodiac() function available');
} else {
    console.warn('ΓÜá∩╕Å findChineseZodiac() not found! Using compact fallback');
    ...
    console.log('Γ£à Compact fallback zodiac lookup loaded (116 years, 1925-2040)');
}

// Load profile data from sessionStorage
document.addEventListener('DOMContentLoaded', function() {
    console.log('\n≡ƒôä Results page DOMContentLoaded event fired');
    console.log('ΓÅ░ Timestamp:', new Date().toISOString());
    
    // Check ALL storage
    console.log('\n≡ƒöì CHECKING ALL STORAGE:');
    console.log('sessionStorage keys:', Object.keys(sessionStorage));
    console.log('localStorage keys:', Object.keys(localStorage));
    
    // Check currentProfile
    const profileDataString = sessionStorage.getItem('currentProfile');
    console.log('\n≡ƒôª sessionStorage.currentProfile:', profileDataString);
    
    if (!profileDataString) {
        console.error('Γ¥î NO PROFILE DATA IN SESSION STORAGE!');
        ...
        if (localProfiles) {
            console.log('≡ƒôï Found profiles in localStorage:', localProfiles);
    ...
    console.log('\n≡ƒÄ¿ Populating page with profile data...');
    populateProfileData(profile);
    
    // Calculate and display all astrological data
    console.log('\n≡ƒö« Calculating astrological data...');
    calculateAndDisplay(profile);
    
    console.log('\nΓ£¿ ALL DONE! Page should now show correct data.');
});

function populateProfileData(profile) {
    console.log('≡ƒô¥ populateProfileData() called');
    ...
    if (birthInfoPanel) {
        ...
        if (infoContent) {
            ...
            // Enhancement: Add birth time explanation if time is unknown
            let birthTimeValue = birthTime;
            if (!profile.birthHour24 && profile.birthHour24 !== 0) {
                birthTimeValue = '<span title="When birth time is unknown, 12:00 PM (noon) is used as the standard midpoint of the day for astrological calculations." style="cursor: help;">⏰ Unknown - using 12:00 PM</span>';
            }
            ...
            infoContent.innerHTML = `
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">${birthTimeValue}</span>
                </div>
                <div class="info-row">
            ...
            `;
            console.log('Γ£à Birth info panel updated (with coordinates & timezone)');
        } else {
            console.error('Γ¥î .info-content not found inside .info-panel');
        }
    } else {
        console.error('Γ¥î .info-panel not found on page');
    }
...
function calculateAge(birthDate) {
    ...
    if (ageDisplay) {
        // Enhancement: Add "As of" date
        const todayFormatted = today.toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
        });
        ageDisplay.innerHTML = `${years} years, ${months} months, ${days} days<br><span style="color: #888; font-size: 0.9em;">(As of ${todayFormatted})</span>`;
        console.log('Γ£à Age calculated:', `${years} years, ${months} months, ${days} days`);
    }
}
...
function calculateAndDisplay(profile) {
    ...
    console.log('≡ƒö« Calculating with:');
    ...
    console.log('Γ£¿ All calculations complete!');
}

function calculateChineseZodiac(year, month, day) {
    console.log(`≡ƒÉë Calculating Chinese Zodiac for ${year}-${month}-${day}`);
    
    // Check if data is available
    if (typeof findChineseZodiac === 'undefined') {
        console.error('Γ¥î findChineseZodiac function not available!');
            ...
            if (zodiacDisplay) {
                zodiacDisplay.innerHTML = `
                    <p style="color: #ef4444; text-align: center;">
                        ΓÜá∩╕Å Error loading zodiac data<br>
    ...
    if (!zodiac) {
        console.error('Γ¥î No zodiac data found for this date');
        return;
    }
...
function getAnimalEmoji(animal) {
    ...
    return emojiMap[animal] || 'Γ£¿';
}

function calculateWesternZodiac(month, day) {
    const signs = [
        { name: 'Capricorn', dates: [[12, 22], [1, 19]], element: 'Earth', symbol: 'ΓÖæ' },
        { name: 'Aquarius', dates: [[1, 20], [2, 18]], element: 'Air', symbol: 'ΓÖÆ' },
        { name: 'Pisces', dates: [[2, 19], [3, 20]], element: 'Water', symbol: 'ΓÖô' },
        { name: 'Aries', dates: [[3, 21], [4, 19]], element: 'Fire', symbol: 'ΓÖê' },
        { name: 'Taurus', dates: [[4, 20], [5, 20]], element: 'Earth', symbol: 'ΓÖë' },
        { name: 'Gemini', dates: [[5, 21], [6, 20]], element: 'Air', symbol: 'ΓÖè' },
        { name: 'Cancer', dates: [[6, 21], [7, 22]], element: 'Water', symbol: 'ΓÖï' },
        { name: 'Leo', dates: [[7, 23], [8, 22]], element: 'Fire', symbol: 'ΓÖî' },
        { name: 'Virgo', dates: [[8, 23], [9, 22]], element: 'Earth', symbol: 'ΓÖì' },
        { name: 'Libra', dates: [[9, 23], [10, 22]], element: 'Air', symbol: 'ΓÖÄ' },
        { name: 'Scorpio', dates: [[10, 23], [11, 21]], element: 'Water', symbol: 'ΓÖÅ' },
        { name: 'Sagittarius', dates: [[11, 22], [12, 21]], element: 'Fire', symbol: 'ΓÖÉ' }
    ];
...
function calculateYinYang(birthDate, birthHour24) {
    ...
    if (panel) {
        ...
        if (yinDesc) {
            ...
            if (h4) h4.textContent = `≡ƒîÖ YIN ENERGY (${yinPercentage}%)`;
            if (p2) p2.textContent = 'Feminine Energy: Receptive, Intuitive, Nurturing';
            if (p3) p3.textContent = `Your ${yinPercentage}% Yin energy represents your receptive, introspective nature. This is the quiet strength of waterΓÇöflowing, adapting, and nurturing the inner world. It's about emotional depth, intuition, and the wisdom of listening.`;
        }
        ...
        if (yangDesc) {
            ...
            if (h4) h4.textContent = `ΓÿÇ∩╕Å YANG ENERGY (${yangPercentage}%)`;
            if (p2) p2.textContent = 'Masculine Energy: Active, Logical, Assertive';
            if (p3) p3.textContent = `Your ${yangPercentage}% Yang energy fuels your active, outward-directed nature. This is the driving force of fireΓÇöinitiating, creating, and shaping the external world. It's about action, logic, and the power of doing.`;
        }
    }
}
...
console.log('\nΓ£à All functions loaded and ready');
