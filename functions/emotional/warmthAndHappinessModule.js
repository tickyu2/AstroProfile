...
class WarmthAndHappinessModule {
  ...
  adviseAction(state, warmthMultiplier) {
    ...
    return {
      ...
      outcomes: {
        ...
        neutral: {
          probability: this.calculateNeutralProbability(state),
          happinessGain: 0,
   ...
   *
   * Probabilities: Success + Neutral + Setback = 100%
   * - Success: 25-50% (based on confidence)
   * - Neutral: 25-50% (remaining split)
   * - Setback: 15-25% (lower end)
   */
  calculateRejectionProbability(state) {
    const success = this.calculateSuccessProbability(state);
    // Base rejection: 25%, decreases as success increases
    const baseRejection = Math.max(15, 50 - success);
   ...
   */
  calculateNeutralProbability(state) {
    const success = this.calculateSuccessProbability(state);
    const setback = this.calculateRejectionProbability(state);
    return 100 - success - setback;
  }
  ...
  calculateExpectedHappiness(state) {
    ...
    const pNeutral = this.calculateNeutralProbability(state) / 100;
  ...
  applyWarmth(multiplier) {
    // Use multiplier directly for level determination
    // (baseline affects final warmth value, not level)
  ...
  updateAdaptation(experienceType, intensity) {
    ...
    const daysSince = lastClimb > 0 ? (now - lastClimb) / (1000 * 60 * 60 * 24) : 0;

    // Natural decay over time (only if there was a previous climb)
    if (lastClimb > 0 && daysSince > 1) {
