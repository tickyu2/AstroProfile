...
function optimizeMemories(ltmResults, stmResults, topN = 10) {
  ...
  const deduplicated = deduplicateMemories(ltmResults, stmResults);
...
function getOptimizationStats(ltmResults, stmResults, optimized) {
  const totalInput = ltmResults.length + stmResults.length;
  const totalOutput = optimized.length;
  const reduction = totalInput - totalOutput;
  
  const ltmInOutput = optimized.filter(m => m.source === 'ltm').length;
  const stmInOutput = optimized.filter(m => m.source === 'stm').length;
  const boostedCount = optimized.filter(m => m.boosted).length;
  
  return {
    input: {
      ltm: ltmResults.length,
      stm: stmResults.length,
      total: totalInput
    },
    output: {
      ltm: ltmInOutput,
