...
export default function Results() {
    ...
    const getFactorTheory = (factor, profileData) => {
        ...
        if (factorName.includes('Gender')) {
            ...
            return yinYangTheory.gender[gender]
        }
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white/5 backdrop-blur-lg rounded-xl p-4 border border-amber-500/30 hover:border-amber-400/60 hover:shadow-[0_0_25px_rgba(255,215,0,0.3)] transition-all duration-300 hover:-translate-y-1 fade-in delay-2">
                        ...
                        {/* BOUNCING RABBIT ON LEFT + TITLE/BADGES ON RIGHT */}
                        <div className="flex items-start gap-3 mb-3">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    ...
                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                                        zodiacProfile.polarity === 'Yin' ? 'bg-blue-500 text-white' : 'bg-amber-500 text-slate-900'
                                    }`}>{zodiacProfile.polarity === 'Yin' ? '🌙' : '☀️'} {zodiacProfile.polarity}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-900/40 rounded-lg p-2.5 mb-2">
                            <div className="text-[10px] font-bold text-amber-300 uppercase tracking-wide mb-1">Layer 1: All {zodiacProfile.animal}s</div>
                            ...
                            <div className="flex flex-wrap gap-1">
                                {zodiacProfile.strengths.slice(0, 3).map((trait, idx) => (
                                    <span key={idx} className="px-1.5 py-0.5 bg-amber-500/20 rounded text-[9px] text-amber-300">{trait}</span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-900/40 rounded-lg p-2.5 mb-3">
                            <div className="text-[10px] font-bold text-purple-300 uppercase tracking-wide mb-1">
                                Layer 3: {zodiacProfile.polarity === 'Yin' ? '🌙' : '☀️'} {zodiacProfile.polarity} Polarity
                            </div>
                            <p className="text-[11px] text-white/90 leading-relaxed">
                                All {zodiacProfile.animal}s are {zodiacProfile.polarity}. {zodiacProfile.polarityDescription}
                            </p>
                        </div>

                        {/* 60-YEAR CYCLE MATRIX - 5 ROWS TOTAL */}
                        <div className="bg-gradient-to-br from-amber-500/10 to-yellow-500/10 rounded-lg p-2.5 mb-3 border border-amber-400/20">
                            ...
                            <div className="space-y-1 text-[11px]">
                                {(() => {
                                    // Calculate 60-year cycle for all 5 elements
                                    ...
                                    const elementData = elementOrder.map((element, idx) => {
                                        ...
                                        return {
                                            ...
                                            centerIndex: 2 // User is always at index 2 (middle position)
                                        }
                                    })
                                    ...
                                    return elementData.map((data, idx) => (
                                        <div key={idx} className={`flex items-center justify-between rounded py-1.5 px-2 transition-all duration-200 ${
                                            ...
                                            {/* Years on RIGHT - all on same line */}
                        ...
                        <button className="w-full py-2.5 bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-900 text-sm font-bold rounded-lg hover:from-amber-600 hover:to-yellow-600 transition-all hover:scale-[1.02] shadow-lg">
                            Explore {zodiacProfile.animal} Compatibility →
                        </button>
                    </div>
                    ...
                    {/* 5. YIN/YANG BALANCE - 100-POINT SYSTEM WITH 3 ENERGIES */}
                    <div className="bg-white/5 backdrop-blur-lg rounded-xl p-4 border border-amber-500/30 hover:border-amber-400/60 hover:shadow-[0_0_25px_rgba(255,215,0,0.3)] transition-all duration-300 hover:-translate-y-1 fade-in delay-5">
                        <div className="flex items-center gap-2 mb-3 pb-2 border-b border-amber-500/30">
                            <span className="text-xl">☯</span>
                            <h2 className="text-sm font-bold text-amber-400">YIN/YANG BALANCE</h2>
                        </div>
                        ...
                        {/* Learn More Button - Toggles Breakdown */}
                        <button 
                        ...
                        >
                            <span>🔍</span>
                            <span>{showYinYangBreakdown ? 'Hide Calculation' : 'See How We Calculated This'}</span>
                        ...
                        {showYinYangBreakdown && (
                            <div className="mt-4 pt-4 border-t border-white/20 animate-fadeIn">
                                <h3 className="text-xs font-bold text-white/90 mb-3 flex items-center gap-2">
                                    ...
                                    <span className="text-white/50 font-normal">({yinYang.factors?.length || 0} factors)</span>
                                </h3>
                                            ...
                                            <div key={idx} className="rounded-lg overflow-hidden">
                                                ...
                                                <div className={`flex items-center justify-between p-2 text-xs ${
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <span>{factor.energy === 'Yin' ? '🌙' : factor.energy === 'Yang' ? '☀️' : '⚖️'}</span>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-white/50">+{factor.weight} pts</span>
                                                        
                                                        {/* Learn Why Button */}
                                                        {theory && (
                                                            <button
                                                                onClick={() => toggleFactorTheory(idx)}
                                                                className={`ml-2 px-2 py-1 text-[10px] rounded transition-all ${
                                                                    isExpanded 
                                ...
                                <div className={`grid ${(yinYang.balancedPoints || 0) > 0 ? 'grid-cols-3' : 'grid-cols-2'} gap-2 mb-3`}>
                                    <div className="bg-blue-500/20 rounded-lg p-2 text-center border border-blue-500/30">
                                        <div className="text-lg font-bold text-blue-300">{yinYang.yinPoints || Math.round(yinYang.yinPercentage || 0)}</div>
                                    ...
                                    {(yinYang.balancedPoints || yinYang.balancedPercentage || 0) > 0 && (
                                        <div className="bg-green-500/20 rounded-lg p-2 text-center border border-green-500/30">
                                            <div className="text-lg font-bold text-green-300">{yinYang.balancedPoints || Math.round(yinYang.balancedPercentage || 0)}</div>
                                            <div className="text-[10px] text-green-400">Balanced</div>
                                        </div>
                                    )}
                                    <div className="bg-amber-500/20 rounded-lg p-2 text-center border border-amber-500/30">
                                        <div className="text-lg font-bold text-amber-300">{yinYang.yangPoints || Math.round(yinYang.yangPercentage || 0)}</div>
                                        <div className="text-[10px] text-amber-400">Yang Points</div>
                                    </div>
                                </div>

                                {/* Formula Explanation - Updated for 100-point system */}
                                <div className="bg-slate-800/50 rounded-lg p-3 border border-white/10">
                                    <p className="text-[10px] text-white/60 leading-relaxed">
                                        <span className="text-white/80 font-semibold">100-Point System:</span> We analyze 7 factors from your birth data 
                                        (Chinese Animal & Element, Western Sign & Element, Planetary Day, Gender, Birth Time). 
                                        Each contributes weighted points out of 100 total. Factors are classified as Yin (receptive), 
                                        Yang (active), or Balanced (adaptable). All points are verifiable - add them up yourself! 
                                        <span className="text-amber-400"> Clean math. Clear authority. ✨</span>
                                    </p>
                                </div>
                            </div>
                        )}
