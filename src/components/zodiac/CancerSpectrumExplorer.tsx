...
const DegreeSlider: React.FC<{
  ...
  const currentZone = getZoneFromDegree(cancerZones, currentDegree);
...
const CancerSpectrumExplorer: React.FC<CancerSpectrumExplorerProps> = ({
  ...
  const handleZoneToggle = (zoneId: number) => {
    ...
    if (viewMode === 'single') {
      ...
      const zone = cancerZones.find((z: Zone) => z.id === zoneId);
  ...
  const displayZones = useMemo(() => {
    ...
    return cancerZones.filter((zone: Zone) => selectedZones.includes(zone.id));
  }, [viewMode, selectedZones]);

  const userZoneInfo = useMemo(
    () => (userDegree != null ? getUserZoneInfo('Cancer', cancerZones, userDegree) : null),
    [userDegree]
  );
