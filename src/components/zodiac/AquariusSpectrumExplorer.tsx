...
const DegreeSlider: React.FC<{
  ...
  const currentZone = getZoneFromDegree(aquariusZones, currentDegree);
...
const AquariusSpectrumExplorer: React.FC<AquariusSpectrumExplorerProps> = ({
  ...
  const handleZoneToggle = (zoneId: number) => {
    ...
    if (viewMode === 'single') {
      ...
      const zone = aquariusZones.find((z: Zone) => z.id === zoneId);
  ...
  const displayZones = useMemo(() => {
    ...
    return aquariusZones.filter((zone: Zone) => selectedZones.includes(zone.id));
  }, [viewMode, selectedZones]);

  const userZoneInfo = useMemo(
    () => (userDegree != null ? getUserZoneInfo('Aquarius', aquariusZones, userDegree) : null),
    [userDegree]
  );
