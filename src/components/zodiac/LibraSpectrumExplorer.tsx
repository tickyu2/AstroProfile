...
const DegreeSlider: React.FC<{
  ...
  const currentZone = getZoneFromDegree(libraZones, currentDegree);
...
const LibraSpectrumExplorer: React.FC<LibraSpectrumExplorerProps> = ({
  ...
  const handleZoneToggle = (zoneId: number) => {
    ...
    if (viewMode === 'single') {
      ...
      const zone = libraZones.find((z: Zone) => z.id === zoneId);
  ...
  const displayZones = useMemo(() => {
    ...
    return libraZones.filter((zone: Zone) => selectedZones.includes(zone.id));
  }, [viewMode, selectedZones]);

  const userZoneInfo = useMemo(
    () => (userDegree != null ? getUserZoneInfo('Libra', libraZones, userDegree) : null),
    [userDegree]
  );
