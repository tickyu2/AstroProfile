/**
 * TaurusSpectrumExplorer.tsx
 *
 * Educational tool for understanding the 30° journey through Taurus.
 * Divides Taurus into 6 zones (5° each) with quality metrics, decan rulers,
...
// @ts-ignore
import { qualityCategories, compareQualityAcrossZones } from '../../data/taurusQualityCategories';
// @ts-ignore
import { getZoneFromDegree, getUserZoneInfo } from '../../utils/zoneCalculations';
import './TaurusSpectrumExplorer.css';
// @ts-ignore
import ThinkingStyleTab from './ThinkingStyleTab';
...
interface TaurusSpectrumExplorerProps {
...
const DegreeSlider: React.FC<{
  ...
  const currentZone = getZoneFromDegree(taurusZones, currentDegree);
  ...
  return (
    <div className="tse-slider-container">
      ...
      <div className="tse-zodiac-context">
        <span className="tse-sign-label tse-aries">{'\u2648'} Aries 29°</span>
        <span className="tse-sign-label tse-taurus-label">{'\u2649'} TAURUS (0° - 30°)</span>
        <span className="tse-sign-label tse-gemini">{'\u264A'} Gemini 0°</span>
      </div>
      ...
      <div className="tse-decan-markers">
        ...
        <div className={`tse-decan tse-decan-2 ${highlightedDecan === 2 ? 'tse-decan-active' : ''}`}
             ...
             title="Second Decan: Venus/Mercury - Fixed (click to highlight zones 3-4)">
        ...
        <div className={`tse-decan tse-decan-3 ${highlightedDecan === 3 ? 'tse-decan-active' : ''}`}
             ...
             title="Third Decan: Venus/Saturn - Mutable (click to highlight zones 5-6)">
      ...
      <div className="tse-zone-markers">
        ...
        {userDegree !== null && userDegree !== undefined && showUserHighlight && (
          <div
            ...
            title={`Your Sun: ${(userDegree as number).toFixed(2)}° Taurus`}
          >
      ...
      <div className="tse-slider-wrapper">
        <input
          ...
          style={{ '--tse-zone-color': currentZone?.colorTheme?.primary || '#8B4513' } as React.CSSProperties}
        />
      </div>
      ...
      <div className="tse-slider-info">
        <div className="tse-current-position">
          <div className="tse-degree-display">
            ...
            <span className="tse-sign-name">Taurus</span>
            <span className="tse-approx-date">~ {degreeToApproxDateTime(30 + currentDegree, ephemerisTimestamps)}</span>
          </div>
        ...
        {currentZone && (
          <div className="tse-current-zone" style={{ '--zone-border-color': currentZone.colorTheme.primary } as React.CSSProperties}>
  ...
  >
    ...
    <header className="tse-szv-header">
      ...
      <div className="tse-degree-info-grid">
        <div className="tse-info-item">
          ...
          <span className="tse-info-value">{zone.degreeRange.start}° - {zone.degreeRange.end}° Taurus</span>
        </div>
        <div className="tse-info-item">
    ...
    <section className="tse-section">
      ...
      <div className="tse-decan-grid">
        ...
        <div className="tse-decan-info-card"><label>Primary Ruler</label><div className="tse-decan-val">{zone.decan.primaryRuler} (Taurus)</div></div>
    ...
    {zone.sabianSymbol && (
      <section className="tse-section">
        ...
        <div className="tse-sabian-card">
          <div className="tse-sabian-degree">{zone.sabianSymbol.degree}° Taurus</div>
      ...
      <section className="tse-commonalities">
        ...
        <div className="tse-commonalities-grid">
          <div className="tse-commonality-item">
            ...
            <div><strong>Loyalty:</strong> All maintain 80-100% loyalty\u2014this is the Taurus constant</div>
          </div>
          <div className="tse-commonality-item">
            <span className="tse-commonality-icon">{'\uD83C\uDF39'}</span>
            <div><strong>Sensuality:</strong> All express some form of physical/aesthetic appreciation</div>
          </div>
          <div className="tse-commonality-item">
            <span className="tse-commonality-icon">{'\uD83C\uDFD7\uFE0F'}</span>
            <div><strong>Building Instinct:</strong> All are oriented toward creating lasting things</div>
          </div>
        </div>
      </section>
    </div>
...
const FullMatrixView: React.FC<{
  ...
  // Degree → date string (Taurus: ~April 20 to May 20, 31 days for 30°)
  const degreeToDateStr = (deg: number) => {
    const d = new Date(2024, 3, 20);
  ...
  const DECANS = [
    { num: 1, name: 'First Decan', ruler: 'Venus / Venus', quality: 'Cardinal · Initiating', accent: '#FF6B35' },
    { num: 2, name: 'Second Decan', ruler: 'Venus / Mercury', quality: 'Fixed · Stabilizing', accent: '#2D5016' },
    { num: 3, name: 'Third Decan', ruler: 'Venus / Saturn', quality: 'Mutable · Transitioning', accent: '#6A5ACD' },
  ];
  ...
  const phiBlend = useMemo(() => {
    ...
    return {
      aries: Array.from({ length: N }, (_, i) => {
        const dt = new Date(2024, 3, 20 + i);
        const a = adj(i + 1);
        return { dt: dt.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), tau: 100 - a, adj: a };
      }),
      gemini: Array.from({ length: N }, (_, i) => {
        const dt = new Date(2024, 4, 15 + i);
        const a = adj(N - i);
        return { dt: dt.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), tau: 100 - a, adj: a };
      }),
    };
  ...
  const exportMD = () => {
    ...
    let md = '# The Taurus Spectrum Explorer\n\n';
    md += '> A 30° journey through Taurus — six zones, three decans, one sign.\n\n';
    ...
    md += '|-------|-------|-------|---------|\n';
    md += '| First (0°–9°) | Venus / Venus | 1–2 | Cardinal · Initiating |\n';
    md += '| Second (10°–19°) | Venus / Mercury | 3–4 | Fixed · Stabilizing |\n';
    md += '| Third (20°–29°) | Venus / Saturn | 5–6 | Mutable · Transitioning |\n';

    // φ Golden Ratio Cusp Blending
    md += '\n## φ Golden Ratio Cusp Blending\n\n';
    md += 'Adjacent sign influence = ((days_remaining ÷ 7)^φ) × 100%, where φ = 1.618033…\n\n';
    md += '### Aries ♈ Cusp (first 6 days)\n\n';
    md += '| Date | Taurus % | Aries % |\n';
    md += '|------|----------|----------|\n';
    for (let i = 0; i < N; i++) {
      const a = adj(i + 1);
      const dt = new Date(2024, 3, 20 + i);
    ...
    md += '\n**Apr 26 – May 14: 100% Pure Taurus (19 days)**\n\n';
    md += '### Gemini ♊ Cusp (last 6 days)\n\n';
    md += '| Date | Taurus % | Gemini % |\n';
    md += '|------|----------|----------|\n';
    for (let i = 0; i < N; i++) {
      const a = adj(N - i);
      const dt = new Date(2024, 4, 15 + i);
    ...
    md += '---\n*Generated by The Taurus Spectrum Explorer · AstroProfile*\n';
    ...
    a.href = url;
    a.download = 'taurus-spectrum-explorer.md';
    a.click();
  ...
  <div className="tse-matrix-view">
    ...
    {matrixFlapOpen && (
      <div className="tse-float-panel" style={{ left: flapPos.x, top: flapPos.y }}>
        ...
        <div className="tse-float-titlebar" onMouseDown={onDragStart}>
          ...
          <h2>The Taurus Spectrum Explorer</h2>
        ...
        <div className="tse-float-zones">
          <div className="tse-float-zones-title">The Complete Taurus Spectrum</div>
        ...
        <div className="tse-float-interactive">
          <div className="tse-float-slider-wrap">
            ...
            <input
              ...
              title="Taurus degree position"
            />
          </div>
          <div className="tse-float-readout">
            <span className="tse-float-readout-deg">{flapDegree.toFixed(1)}° Taurus</span>
            ...
            <div className="tse-phi-body">
              <div className="tse-phi-chart">
                ...
                <div className="tse-phi-wing">
                  <div className="tse-phi-wing-hd" style={{ color: '#ef4444' }}>{'\u2648'} Aries Influence ({'\u03C6'})</div>
                    ...
                    {phiBlend.aries.map((d, i) => (
                      ...
                      >
                        ...
                        <div className="tse-phi-vb">
                          ...
                          <div className="tse-phi-vb-t" style={{ height: `${d.tau}%`, background: '#22c55e' }} />
                        </div>
                        <span className="tse-phi-t">{d.tau}%</span>
                ...
                <div className="tse-phi-mid">
                  ...
                  <span className="tse-phi-mid-range">Apr 26 {'\u2013'} May 14</span>
                  <span className="tse-phi-mid-sub">19 days pure Taurus</span>
                </div>
        ...
        <div className="tse-float-table-scroll">
          ...
          {matrixTab === 'thinking' && (
            <ThinkingStyleTab zones={zones} userDegree={userDegree} />
          )}
        </div>
      </div>
    )}
    ...
    <section className="tse-decan-structure">
      ...
      <div className="tse-decan-row">
        ...
        <div className="tse-decan-block tse-db-2">
          ...
          <div className="tse-db-ruler">Mercury/Virgo Ruled</div>
    ...
    <section className="tse-pattern-insights">
      ...
      <div className="tse-insights-grid">
        {[
          { title: 'Speed Spectrum', text: 'Zone 1 moves 4x faster than Zone 3 (80% vs 20%), with gradual acceleration from Zone 3\u21926 as Gemini influence builds.' },
          { title: 'Sensuality Peak', text: 'Zone 3 (10-14°) represents peak Taurus sensuality (100%), with decline at both cusp ends due to Aries/Gemini influences.' },
          { title: 'Stubbornness Arc', text: 'Highest in Zone 3 (110%), decreasing toward both cusps as adaptability increases from neighboring sign influences.' },
          { title: 'Ambition Rise', text: 'Ambition steadily increases from Zone 1\u21925 (peaking at Zone 5 with Saturn influence) before shifting to communication in Zone 6.' },
          { title: 'Loyalty Constant', text: 'All zones maintain 80-110% loyalty despite other differences\u2014this is the non-negotiable Taurus core trait.' },
          { title: 'Energy Distribution', text: 'Physical energy peaks in Zone 1 (100% with Aries influence), normalizes in middle zones, then rises slightly in Zone 6 (Gemini mental energy).' },
        ].map(({ title, text }) => (
    ...
    <section className="tse-evolution-narrative">
      ...
      <div className="tse-narrative-content">
        <p><strong>Zone 1 (0-4°):</strong> The Warrior-Builder charges into Taurus season with Aries fire still burning, initiating projects with high energy and quick decisions.</p>
        <p><strong>Zone 2 (5-9°):</strong> Pure Venus energy emerges as Aries fades. The Builder focuses on aesthetics, sensuality, and creating beautiful foundations.</p>
        <p><strong>Zone 3 (10-14°):</strong> Peak Taurus. Maximum stubbornness, patience, and sensuality. This is the Bull at full strength\u2014immovable, deeply rooted, eternally loyal.</p>
        <p><strong>Zone 4 (15-19°):</strong> Mercury's influence adds precision. The Craftsperson perfects technique through analysis, detail-orientation, and dedicated practice.</p>
        <p><strong>Zone 5 (20-24°):</strong> Saturn brings ambition. The Empire Builder thinks in generations, creating structures and legacies that transcend individual lifetimes.</p>
        <p><strong>Zone 6 (25-29°):</strong> Gemini's approach adds communication. The Articulate Builder can explain, network, and adapt while maintaining core Taurus values.</p>
      </div>
    </section>
  </div>
  );
...
const TaurusSpectrumExplorer: React.FC<TaurusSpectrumExplorerProps> = ({
  userDegree = null,
  userName = null,
  ...
  useEffect(() => {
    const currentZone = taurusZones.find(
  ...
  const handleZoneToggle = (zoneId: number) => {
    ...
    if (viewMode === 'single') {
      ...
      const zone = taurusZones.find((z: Zone) => z.id === zoneId);
  ...
  const displayZones = useMemo(() => {
    if (viewMode === 'matrix') return taurusZones;
    return taurusZones.filter((zone: Zone) => selectedZones.includes(zone.id));
  }, [viewMode, selectedZones]);

  const userZoneInfo = useMemo(
    () => (userDegree != null ? getUserZoneInfo('Taurus', taurusZones, userDegree) : null),
    [userDegree]
  );
  ...
  return (
    <div className="taurus-spectrum-explorer">
      ...
      <header className="tse-header">
        ...
        <p className="tse-subtitle">Understanding the 30° Journey Through the Bull</p>
        {userName && userDegree != null && (
          <div className="tse-user-welcome">
            Welcome, {userName}! Your Sun is at {(userDegree as number).toFixed(2)}° Taurus
