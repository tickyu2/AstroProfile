...
const DegreeSlider: React.FC<{
  ...
  const currentZone = getZoneFromDegree(geminiZones, currentDegree);
...
const GeminiSpectrumExplorer: React.FC<GeminiSpectrumExplorerProps> = ({
  ...
  const handleZoneToggle = (zoneId: number) => {
    ...
    if (viewMode === 'single') {
      ...
      const zone = geminiZones.find((z: Zone) => z.id === zoneId);
  ...
  const displayZones = useMemo(() => {
    ...
    return geminiZones.filter((zone: Zone) => selectedZones.includes(zone.id));
  }, [viewMode, selectedZones]);

  const userZoneInfo = useMemo(
    () => (userDegree != null ? getUserZoneInfo('Gemini', geminiZones, userDegree) : null),
    [userDegree]
  );
