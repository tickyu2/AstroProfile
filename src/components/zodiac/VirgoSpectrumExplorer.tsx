...
const DegreeSlider: React.FC<{
  ...
  const currentZone = getZoneFromDegree(virgoZones, currentDegree);
...
const VirgoSpectrumExplorer: React.FC<VirgoSpectrumExplorerProps> = ({
  ...
  const handleZoneToggle = (zoneId: number) => {
    ...
    if (viewMode === 'single') {
      ...
      const zone = virgoZones.find((z: Zone) => z.id === zoneId);
  ...
  const displayZones = useMemo(() => {
    ...
    return virgoZones.filter((zone: Zone) => selectedZones.includes(zone.id));
  }, [viewMode, selectedZones]);

  const userZoneInfo = useMemo(
    () => (userDegree != null ? getUserZoneInfo('Virgo', virgoZones, userDegree) : null),
    [userDegree]
  );
