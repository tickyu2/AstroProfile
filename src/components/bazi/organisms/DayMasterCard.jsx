 ...
 * Now includes Yin/Yang polarity gauge and stem-branch rootedness meter
 */
...
import DmPolarityGauge from '../molecules/DmPolarityGauge';
import DmRootednessMeter from '../molecules/DmRootednessMeter';
import { computeDmPolarityAndRootedness } from '../../../utils/baziDmPolarityRootedness';
...
const DayMasterCard = ({
  ...
  showRootednessMeter = true,
  compact = false
}) => {
  const theme = useBaziTheme();

  // Compute polarity and rootedness if pillars provided
  const polarityRootedness = useMemo(() => {
    if (!pillars) return null;
    try {
      return computeDmPolarityAndRootedness(pillars);
    } catch (e) {
      console.warn('Could not compute polarity/rootedness:', e);
      return null;
    }
  }, [pillars]);
  ...
  return (
    <div style={styles.card}>
      ...
      {/* Polarity & Rootedness Gauges (if pillars provided) */}
      {polarityRootedness && (showPolarityGauge || showRootednessMeter) && (
        <div style={styles.gaugesGrid}>
          {showPolarityGauge && (
            <DmPolarityGauge
              yinRatio={polarityRootedness.yinRatio}
              yangRatio={polarityRootedness.yangRatio}
              dominant={polarityRootedness.dominant}
              dmPolarity={polarityRootedness.yinYangProfile.dmPolarity}
              dmPolarityMatch={polarityRootedness.yinYangProfile.dmPolarityMatch}
              polarityLabel={polarityRootedness.polarityLabel}
              compact={compact}
            />
          )}
          {showRootednessMeter && (
            <DmRootednessMeter
              rootednessNormalized={polarityRootedness.rootednessNormalized}
              rootednessLabel={polarityRootedness.rootednessLabel}
              dmHasStrongRoot={polarityRootedness.stemBranch.dmHasStrongRoot}
              rootedPillarCount={polarityRootedness.stemBranch.rootedPillarCount}
              compact={compact}
            />
          )}
        </div>
