...
// Helper functions for working with Leo degree placements

import leoZones from '../data/leoZones.js';

/**
 * Get the zone for a given Leo degree
 * @param {number} degree - Degree within Leo (0-29.99)
 * @returns {object|null} - Zone object or null if invalid
 */
export function getZoneFromDegree(degree) {
  if (degree < 0 || degree >= 30) {
    console.warn(`Invalid Leo degree: ${degree}. Must be 0-29.99`);
    return null;
  }
 ...
 * @param {number} leoDegree - Degree within Leo (0-29.99)
 * @returns {number} - Absolute ecliptic longitude (120-149.99)
 */
export function getAbsoluteEclipticPosition(leoDegree) {
  return 120 + leoDegree;
}
...
export function getCuspInfluences(degree) {
  ...
  // Cancer cusp influence (0-5\u00B0)
  if (degree < 5) {
    const cancerInfluence = Math.round((5 - degree) / 5 * 25);
    influences.cancer = cancerInfluence;
    influences.leo = 100 - cancerInfluence;
  }

  // Virgo cusp influence (25-30\u00B0)
  if (degree >= 25) {
    const virgoInfluence = Math.round((degree - 25) / 5 * 25);
    influences.virgo = virgoInfluence;
    influences.leo = 100 - virgoInfluence;
  }
...
export function getDateRangeForDegree(degree) {
  const startDate = new Date(2024, 6, 23); // July 23, 2024 (Leo start approx)
  const daysIntoLeo = degree;

  const birthDate = new Date(startDate);
  birthDate.setDate(startDate.getDate() + Math.floor(daysIntoLeo));
...
function generateQualityComparison(qualityId, range, max, min) {
  const qualityNames = {
    ...
    dramaticFlair: 'dramatic flair',
    needForRecognition: 'need for recognition',
    pride: 'pride',
    loyalty: 'loyalty',
    speed: 'speed'
  };
  ...
  if (range < 20) {
    ...
    return `Significant ${qualityName} difference: ${range}% range shows distinct creative patterns`;
  } else {
 ...
 * @param {number} userDegree - User's Leo Sun degree
...
function generatePersonalizedSuggestion(zone, degree, influences) {
  ...
  if (influences.cancer > 15) {
    suggestion += `With ${influences.cancer}% Cancer influence, you're more emotionally intelligent and nurturing than typical Leo. `;
  }

  if (influences.virgo > 15) {
    suggestion += `With ${influences.virgo}% Virgo influence, you're more analytical and perfectionist than typical Leo. `;
  }

  if (zone.id === 1) {
    suggestion += "You're the Leo who NURTURES\u2014combining fire's creative confidence with water's emotional warmth.";
  } else if (zone.id === 2) {
    suggestion += "You're peak Leo\u2014the most quintessentially solar expression possible, pure creative radiance.";
  } else if (zone.id === 6) {
    suggestion += "You're the Leo who PERFECTS\u2014combining fire's creative vision with earth's analytical precision.";
  }
...
/**
 * Calculate compatibility between two Leo degrees
 * @param {number} degree1 - First person's Leo degree
 * @param {number} degree2 - Second person's Leo degree
 * @returns {object} - Compatibility analysis
 */
export function calculateLeoCompatibility(degree1, degree2) {
...
function getCompatibilityInterpretation(score) {
  if (score >= 90) return "Extremely high compatibility\u2014almost identical Leo expression";
  if (score >= 80) return "High compatibility\u2014minor differences in creative style";
  if (score >= 70) return "Good compatibility\u2014complementary fire energies";
  if (score >= 60) return "Moderate compatibility\u2014need conscious creative bridging";
  if (score >= 50) return "Challenging but workable\u2014significant adaptation needed";
  return "Low compatibility\u2014fundamentally different Leo expressions";
}
